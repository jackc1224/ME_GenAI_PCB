...
def read_dxf_doc_from_bytes(b: bytes):
    """
    穩定讀取 DXF：
    1. 先嘗試用 StringIO 讀文字 DXF。
    2. 若失敗，寫入暫存 .dxf，再用 ezdxf.readfile() 讀取。
    """
    tmp = None
...
def dxf_to_svg(b: bytes) -> str:
    ...
    修正重點：
    1. 原本只用 ezdxf.read(io.StringIO(...))，很多 DXF 會失敗。
    2. 新版會先嘗試 StringIO。
    3. 若失敗，會自動寫入暫存 .dxf 檔，再用 ezdxf.readfile() 讀取。
    4. 支援 LINE、LWPOLYLINE、POLYLINE、CIRCLE、ARC。
    5. 若成功，前端的「原始 PCB 外形」會顯示。
    """
    try:
        doc = read_dxf_doc_from_bytes(b)
    except Exception:
    ...
    els.append(
        '<rect x="0" y="0" width="{}" height="{}" fill="#0b0f18"/>'.format(
            vw, vh
        )
    )
    ...
    for it in items:
        if it[0] == "L":
            ...
            els.append(
                '<line x1="{}" y1="{}" x2="{}" y2="{}" '
                'stroke="#f7e600" stroke-width="1.2" fill="none"/>'.format(
            ...
            els.append(
                '<circle cx="{}" cy="{}" r="{}" '
                'stroke="#ff66cc" stroke-width="1.1" fill="none"/>'.format(
    ...
    els.append(
        ...
        'stroke="#334155" stroke-width="1" fill="none" rx="4"/>'.format(
            m / 2,
            m / 2,
            vw - m,
            vh - m,
...
def gen_dxf(pcb_l, pcb_w, p, name):
    ...
    # 重要修正：
    # ezdxf doc.write() 寫出的是 str，不能直接寫入 BytesIO。
    # 先寫到 StringIO，再 encode 成 bytes，才能給 MinIO put_object 使用。
...
def panel_svg(pcb_l, pcb_w, p):
    ...
    for mx, my in [(2.5, tw - 2.5), (tl - 2.5, tw - 2.5), (2.5, 2.5)]:
        ...
        els.append(
            '<circle cx="{}" cy="{}" r="{}" fill="#e05c5c"/>'.format(
                tx(mx),
                ty(my),
                r_inner,
            )
...
def _report(inp):
    ...
    if inp.get("has_bga"):
        special.append("- BGA/QFN：局部Mark點精度需±0.025mm")
    if inp.get("is_irregular"):
        special.append("- 異形板：強制Tab分板，Tab寬2.0mm，郵票孔直徑0.8mm")
